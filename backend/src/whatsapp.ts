// Regras sobre o formato das mensagens da Evolution/WhatsApp. São funções puras, sem banco, e têm testes.
...
function detectType(message: Record<string, any>): string | undefined {
...
export function parseContent(msg: Pick<WaMessage, 'message' | 'messageType'>): ParsedContent | null {
  ...
  const result = (t: MessageType, text: string | null = null, fileName: string | null = null): ParsedContent => ({
    type: t,
    text: text || null,
  ...
  switch (type) {
    case 'conversation':
      return typeof message.conversation === 'string' && message.conversation ? result('text', message.conversation) : null;
