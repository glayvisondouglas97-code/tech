...
import { removeMediaFiles } from '../whatsapp/media';
import { deleteWhatsappData, whatsappDataOf } from '../whatsapp/privacy';
import { publishReload } from '../whatsapp/realtime';
...
export async function exportSubjectData(db: Db, admin: AuthUser, phone: string, ip: string | null) {
  ...
  return {
    ...
    mensagens_whatsapp: await whatsappDataOf(db, phone),
  };
}

/**
 * Apaga as cópias cruas dos dados da pessoa: linhas recusadas de importação e as conversas de WhatsApp.
 * Devolve os arquivos de mídia, que são apagados do disco depois que a transação confirmar.
 */
async function scrubRawCopies(trx: Db, phone: string): Promise<string[]> {
  ...
  return deleteWhatsappData(trx, phone);
}
...
export async function anonymizeSubject(
  ...
  const result = await db.transaction().execute(async (trx) => {
    ...
    mediaPaths = await scrubRawCopies(trx, phone);
    await audit(trx, {
      userId: admin.id,
