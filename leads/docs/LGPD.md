...
**Dados tratados:** nome, telefone e colunas extras das planilhas importadas; resultado dos contatos e observações; dados de login da equipe.
...
- **Consultar** e **baixar** tudo o que existe sobre a pessoa (JSON com registros e histórico) — acesso e portabilidade.
- **Anonimizar:** apaga nome, telefone, colunas extras, observações e cópias cruas (linhas rejeitadas de importação, webhooks); mantém só os números agregados.
- **Excluir** de vez os registros e o histórico.
...
- Linhas recusadas de importação: 90 dias. Eventos brutos do webhook do WhatsApp: 30 dias.
- Sessões expiradas e links de senha usados/vencidos: removidos periodicamente.
- Leads: ficam até a empresa arquivar/excluir a lista ou atender pedido do titular. Defina uma política (ex.: excluir listas com mais de 12 meses) e use **Listas › Excluir**.

## O que cabe à empresa
- Base legal para contatar os leads (ex.: consentimento ou legítimo interesse) e aviso de privacidade.
- Contrato de operador com a hospedagem (Render/Neon têm termos de processamento de dados).
