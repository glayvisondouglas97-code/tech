...
| Hospedagem | Render (blueprint `render.yaml`) | Deploy por clique, HTTPS e backups do banco inclusos; custo baixo. Docker para qualquer outro lugar. |
...
- `blocked_phones` (não contatar), `message_templates`, `settings`, `audit_log` (acessos e ações sensíveis), `whatsapp_webhook_events`.
...
- **WhatsApp:** o botão usa o link oficial `wa.me/<número>?text=...`. No celular abre direto o aplicativo; no computador, o WhatsApp Desktop ou o Web. Um só formato para todo aparelho, sem opção para configurar.
...
- Integração completa com a WhatsApp Business Cloud API (envio de modelos aprovados); hoje só o webhook de respostas/status, desligado por padrão ([WHATSAPP-CLOUD-API.md](WHATSAPP-CLOUD-API.md)).
