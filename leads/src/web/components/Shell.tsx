...
const NAV: NavItem[] = [
  { to: '/chamar', label: 'A chamar', icon: <IconInbox />, count: true, group: 'work' },
  { to: '/chamados', label: 'Já chamados', icon: <IconCheckCircle />, group: 'work' },
  ...
  const link = (n: NavItem) => (
    ...
    >
      <span className="side-ic">
        {n.icon}
        {n.count && mine > 0 && <span className="side-count">{mine > 99 ? '99+' : mine}</span>}
      </span>
            ...
            .map((n) => (
              ...
              >
                <span className="tabbar-ic">
                  {n.icon}
                  {n.count && mine > 0 && <span className="count">{mine > 99 ? '99+' : fmtN(mine)}</span>}
                </span>
                {n.label}
              </NavLink>
            ))}
